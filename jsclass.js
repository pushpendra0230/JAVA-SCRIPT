...
// // Ques: kya isme sabhi tarh ke users ko hi  lekar unquie purchases kya hai? 
...
//   let titlesGenres=movies.map(movie=>({title: movie.title,genres: movie.genres}));
//   console.log(titlesGenres);
// let titlescastcount=movies.map(movie=>({title: movie.title,castCount: movie.cast.length}));
// console.log(titlescastcount);
// let titlesratings=movies.map(movie=>({title: movie.title,rating: movie.rating*10}));
// console.log(titlesratings);
// let classicMovies = movies.map(movie => ({title: movie.title,isClassic:movie.year<2010}));
// console.log(classicMovies);
// let firstgenres=movies.map(movie=>({title: movie.title,firstGenre:movie.genres[0]}));
// console.log(firstgenres);  
// let moviesuppercase=movies.map(movie=>({title: movie.title,cast:movie.cast.map(actor=>actor.toUpperCase())}));
// console.log(moviesuppercase);
// let moviedescriptions=movies.map(movie=>({title: movie.title,description: `${movie.title} (${movie.year})`}));
// console.log(moviedescriptions);
// let actors={};
// movies.forEach(movie=>{movie.cast.forEach(actor=>{actors[actor]=(actors[actor]||0)+1;});});
// let moviescount=Object.entries(actors).map(([actor,count])=>({actor,movieCount:count}));
// console.log(moviescount);
// let moviesyear=movies.reduce((acc,movie)=>{if(!acc[movie.year]){acc[movie.year]=[]};
...
//     {
//       ratingGroups["1-5"].push(movie.title);
//     }
//     else if(movie.rating>5& movie.rating<=7)
//     {
//       ratingGroups["5.1-7"].push(movie.title);
//     }
//     else if(movie.rating>7&&movie.rating<=9) 
//     {
//       ratingGroups["7.1-9"].push(movie.title);
//     }
//     else if(movie.rating>9&&movie.rating<=10)
//     {
//       ratingGroups["9.1-10"].push(movie.title);
//     }
//   });
